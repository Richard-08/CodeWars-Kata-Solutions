...
function compareVersions (v1, v2) {
    ...
    for (let i = 0; i < Math.max(ver1.length, ver2.length); i += 1) {
      if (parseInt(ver1[i] || 0) < parseInt(ver2[i] || 0)) {
        return false;
      }
    }
    return true;
...
function compareVersions (v1, v2) {
    const ver1 = v1.split('.').map(value => Number(value));
    const ver2 = v2.split('.').map(value => Number(value));
    let count = 0;
    for (let i = 0; i < Math.max(ver1.length, ver2.length); i += 1) {
      if (ver1[i] > ver2[i] || ver2[i] === undefined) {
        count += 1;
      } else if (ver1[i] < ver2[i] || ver1[i] === undefined) {
        count -= 1;
      }
      if (count !== 0) {
        return count > 0;
