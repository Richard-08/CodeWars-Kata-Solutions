...
function isLanguageDiverse(list) {
    const count = list.map(value => list.filter(e => e['language'] === value['language']).length);
    return Math.max(...count) / Math.min(...count) > 2 ? false : true;
  }
...
function isLanguageDiverse(list) {
    ...
    list.forEach(element => {
      if (countLang[element.language]) {
        countLang[element.language] += 1;
      } else {
        countLang[element.language] = 1;
      }
    })
    const arr = Object.values(countLang);
    return Math.max(...arr) / Math.min(...arr) > 2 ? false : true
  }